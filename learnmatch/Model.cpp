...
//void Model::Init(const Config& config, const cv::Mat& image, const IntRect& rect)
//{
...
//	//for(int i=1;i<100;i++)
//	//{
//	//	tempPoints.push_back(Point(i,1));
//	//}
//
//	goodFeaturesToTrack(image,tempPoints,3000,0.01,10.0,Mat());
//	memset(image.data,0,image.total());
//	for (int i=0;i<tempPoints.size();i++)
//	{
//		*(image.data+tempPoints.at(i).y*image.cols+tempPoints.at(i).x)= 255;
//	}
//	imshow("model::init",image);
//	waitKey(0);
//}
	...
	m_pDescriptorExtractor(0)
{
 	...
 	// use the strongest keypoint locations
	std::vector<KeyPoint> keypoints(30000);	
	m_pFeatureDetector->detect(image, keypoints);
...
#if LEARNER_TYPE == BOOSTING
	std::vector<KeyPoint> keypointsKept;
